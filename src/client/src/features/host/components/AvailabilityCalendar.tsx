...
import { ChevronLeft, ChevronRight, X, Save, ArrowLeft, Calendar as CalendarIcon } from 'lucide-react';
import type { Room, RoomAvailability } from '../../../types/host';
import { getRoomAvailability, updateRoomAvailability } from '../../../services/hostService';
...
export default function AvailabilityCalendar({ room, onBack }: AvailabilityCalendarProps) {
  ...
  const handleDayClick = (date: Date) => {
    if (!selectionStart || (selectionStart && selectionEnd)) {
      ...
      // Pre-fill price from the first selected day
      const data = getDayData(selectionStart);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update each date
      // Ideally backend should support bulk update, but we loop for now
      await Promise.all(datesToUpdate.map(dateStr => 
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-900">Availability Calendar</h2>
            <p className="text-slate-500 font-medium">{room.name} - Base Price: ${room.basePrice}</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm select-none">
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isPast ? 'bg-slate-50 border-slate-100 text-slate-300 cursor-not-allowed' : 'cursor-pointer hover:border-brand-accent'}
                    ${selected ? 'bg-brand-accent text-white border-brand-accent shadow-lg scale-105 z-10' : ''}
                    ${!selected && !isAvailable && !isPast ? 'bg-red-50 border-red-200' : ''}
                    ${!selected && isAvailable && !isPast ? 'bg-white border-slate-200' : ''}
                  `}
                >
                  <span className={`text-sm font-bold ${!selected && !isAvailable && !isPast ? 'text-red-500' : ''}`}>
                    {date.getDate()}
                  </span>
                  {!isPast && (
                    <div className={`text-xs font-medium mt-1 ${selected ? 'text-white/90' : 'text-slate-500'}`}>
                      ${price}
                    </div>
                  )}
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 text-center text-sm text-slate-500">
          Click a date to select start, click another to select end. Range selection supported.
        </div>
      </div>
              ...
              <div>
                ...
                <div className="flex gap-4">
                  <label className="flex-1 cursor-pointer">
                    ...
                    />
                    <div className="text-center py-3 rounded-xl border-2 border-slate-200 peer-checked:border-green-500 peer-checked:bg-green-50 peer-checked:text-green-700 font-bold transition-all">
                      Open for Booking
                    </div>
                  </label>
                  <label className="flex-1 cursor-pointer">
                    ...
                    />
                    <div className="text-center py-3 rounded-xl border-2 border-slate-200 peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-700 font-bold transition-all">
                      Closed
                    </div>
                  </label>
                </div>
