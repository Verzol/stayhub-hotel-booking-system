...
import type { PromotionDTO, Hotel } from '../../../types/host';
import { createPromotion } from '../../../services/hostService';
...
export default function PromotionForm({ hotel, onSuccess, onCancel }: PromotionFormProps) {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<PromotionDTO>({
    defaultValues: {
      isActive: true,
      maxUsage: 100,
    }
  ...
  const onSubmit = async (data: PromotionDTO) => {
    try {
      // Ensure dates are in ISO format if needed, but input type="datetime-local" returns string
      // Backend expects LocalDateTime, so ISO string is usually fine.
    ...
    <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-bold text-slate-900">Create New Promotion</h2>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              type="number"
              {...register('discountPercent', { required: 'Required', min: 1, max: 100, valueAsNumber: true })}
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Max Discount Amount ($)</label>
            <input
              type="number"
              {...register('maxDiscountAmount', { required: 'Required', valueAsNumber: true })}
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-accent focus:border-transparent outline-none transition-all"
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Max Usage Limit</label>
            <input
              type="number"
              {...register('maxUsage', { required: 'Required', valueAsNumber: true })}
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-accent focus:border-transparent outline-none transition-all"
            />
          </div>
          ...
          <div className="flex items-center h-full pt-6">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="font-bold text-slate-700">Active immediately</span>
            </label>
          </div>
        </div>
